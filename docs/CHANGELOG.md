...
2026-09-17 | v1.8.1 / 0035 | 修复：macOS 控制栏遮挡视频与错误提示显隐 | 前端（仅 macOS 布局）

- 控制栏：保留 Liquid Glass 玻璃胶囊视觉，但恢复为 Flex 文档流中的底部区域；默认窗口与最小窗口下均由视频区自动让位，不再覆盖画面或字幕
- 错误提示：增加 `#error-tip[hidden]` 强制隐藏规则，并移除 `.playing` 状态下无条件隐藏错误层的冲突规则；未出错时不误显示，播放失败时可正常显示原因
- 平台范围：新增规则均以平台选择器隔离；Android 布局、行为与原生工程保持不变
