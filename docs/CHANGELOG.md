...
2026-09-02 | v1.1.2 / 0013 | 修复：系统快捷键调音量软件条不动 | Rust

- 根因：F11/F12 调节的是 CoreAudio VirtualMasterVolume（虚拟主音量），此前监听的是物理声道 VolumeScalar，系统不一定同步写回声道值导致监听不触发（间歇性）

- 修复：读写与监听全部以 VirtualMasterVolume 为主（声道值兜底），监听同时注册 vmvc 与左右声道

2026-09-02 | v1.1.1 / 0012 | 修复：双击当前视频重置进度 + 进度条首次点击失效 | 前端

- 点击/双击当前正在播放的视频不再从头重播（0003 为循环模式加的同文件重播逻辑移除；单文件组的列表循环改为 ended 直接重播，不受影响）

- 进度条首次点击"弹回原位不生效"根因：input 后 timeupdate 每 250ms 回写 thumb 位置，change 读到被重置的值；修复为拖动/点击期间（seeking 标志）暂停回写
